...
SymmetryFunction::SymmetryFunction(int num_prms, double* prms_i, CutoffFunction* cutfun_i):
cutfun(cutfun_i){
...
{
  return pow(2, -prms[1] + 1)*pow(prms[0]*cos(theta), prms[1])*cutfun->eval(rij)*cutfun->eval(rik)*exp(-prms[2]*(pow(rij, 2) + pow(rik, 2)));
};

double BehlerG4::drij(double rij, double rik, double theta)
{
  return -2*pow(2, -prms[1] + 1)*prms[2]*rij*pow(prms[0]*cos(theta), prms[1])*cutfun->eval(rij)*cutfun->eval(rik)*exp(-prms[2]*(pow(rij, 2) + pow(rik, 2))) + pow(2, -prms[1] + 1)*pow(prms[0]*cos(theta), prms[1])*cutfun->derivative(rij)*cutfun->eval(rik)*exp(-prms[2]*(pow(rij, 2) + pow(rik, 2)));
};

double BehlerG4::drik(double rij, double rik, double theta)
{
  return -2*pow(2, -prms[1] + 1)*prms[2]*rik*pow(prms[0]*cos(theta), prms[1])*cutfun->eval(rij)*cutfun->eval(rik)*exp(-prms[2]*(pow(rij, 2) + pow(rik, 2))) + pow(2, -prms[1] + 1)*pow(prms[0]*cos(theta), prms[1])*cutfun->derivative(rik)*cutfun->eval(rij)*exp(-prms[2]*(pow(rij, 2) + pow(rik, 2)));
};

double BehlerG4::dtheta(double rij, double rik, double theta)
{
  return -pow(2, -prms[1] + 1)*prms[1]*pow(prms[0]*cos(theta), prms[1])*cutfun->eval(rij)*cutfun->eval(rik)*exp(-prms[2]*(pow(rij, 2) + pow(rik, 2)))*sin(theta)/cos(theta);
};
